...
def get_models():
  if not MODELS:
    ...
    MODELS["diab_prep"] = joblib.load(BASE_DIR / "diabetes" / "model" / "preprocessor.sav")
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .icon-report { background: linear-gradient(135deg, #2563eb, #1d4ed8); }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p class="subtitle">Cổng truy cập trực tuyến trải nghiệm 3 ứng dụng dự đoán Machine Learning độc lập và Báo cáo nghiên cứu khoa học toàn diện.</p>
    </div>
    ...
    <div class="grid">
      ...
      <!-- App 4: Research Report -->
      <div class="card">
        <div>
          <div class="card-icon icon-report">DOC</div>
          <h2>Báo Cáo Nghiên Cứu</h2>
          <p>Toàn văn báo cáo khoa học 68 trang đầy đủ 10 chương mục, 25 đồ thị phân tích chất lượng cao, công thức toán học MathJax và thảo luận 18 câu hỏi chuyên sâu.</p>
        </div>
        <div>
          <div class="card-meta">Định dạng: Trực tuyến + PDF chuẩn in ấn</div>
          <a href="/report" class="btn btn-secondary">Đọc Báo Cáo Trực Tuyến &rarr;</a>
        </div>
      </div>
    </div>
...
NAV_BAR_HTML = """
<div style="width: 100%; max-width: 860px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
  <a href="/" style="display: inline-flex; align-items: center; gap: 8px; text-decoration: none; color: #475569; font-weight: 600; font-size: 0.9rem; padding: 8px 14px; background: rgba(255,255,255,0.8); border: 1px solid #cbd5e1; border-radius: 8px; backdrop-filter: blur(8px);">
    &larr; Quay lại Cổng Trung Tâm (Hub)
  </a>
  <a href="/report" style="display: inline-flex; align-items: center; gap: 8px; text-decoration: none; color: #0284c7; font-weight: 600; font-size: 0.9rem; padding: 8px 14px; background: rgba(255,255,255,0.8); border: 1px solid #cbd5e1; border-radius: 8px; backdrop-filter: blur(8px);">
    Xem Báo Cáo Khoa Học &rarr;
  </a>
</div>
"""
...
@app.route("/report")
def web_report():
  return serve_web_page(BASE_DIR / "BAO_CAO_ASSIGNMENT_02.html")

@app.route("/report_images/<path:filename>")
def serve_report_image(filename):
  return send_from_directory(BASE_DIR / "report_images", filename)
      ...
      "knowledge": [
        {
          ...
          "items": [
            {"title": "Kiểm soát Carbohydrate", "content": "Hạn chế đường tinh luyện, nước ngọt có ga và bánh kẹo."},
            {"title": "Bổ sung chất xơ", "content": "Tăng cường rau xanh họ cải và ngũ cốc nguyên hạt."}
          ]
        }
      ]
    })
